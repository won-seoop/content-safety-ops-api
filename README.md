...
UI의 `부하테스트 / 관측` 영역에서 요청 수, 동시성, cached/unique 모드를 선택해 부하테스트를 실행할 수 있습니다. 실행 결과로 RPS, 평균 지연, p95 지연, 에러 수, RSS 메모리, DB 커넥션풀 checked-out/overflow, Redis ping, 병목 힌트를 확인할 수 있습니다.
